...
class GhostCardUI:
    ...
    def get_strategy_opening(strategy) -> Tuple[str, str]:
        """根据AI策略获取开局文本和缩略图URL"""
        opening_text = text_config.opening.ai_strategy_text.get(strategy.name, "游戏开始")
    ...
    def create_game_embed(game_id: str) -> discord.Embed:
        ...
        embed.add_field(
            ...
            value=" ".join([GhostCardUI.CARD_TO_EMOJI.get(card, card) for card in game["player_hand"]]) if game["player_hand"] else "无",
