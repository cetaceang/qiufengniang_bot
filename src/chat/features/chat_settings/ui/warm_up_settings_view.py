...
class WarmUpSettingsView(View):
    ...
    @classmethod
    async def create(cls, interaction: Interaction, parent_view_message: discord.Message):
        """工厂方法，用于异步创建和初始化View。"""
    ...
    def _create_view_items(self):
        ...
        if self.paginator:
            ...
            self.add_item(discord.ui.Button(label="服务器内没有论坛频道", disabled=True, row=0))

        # 将返回按钮放到第 2 行
        back_button = discord.ui.Button(label="返回主菜单", style=discord.ButtonStyle.gray, custom_id="back_to_main", row=2)
    ...
    async def on_selection(self, interaction: Interaction):
        ...
        # 获取当前页面所有的选项ID
        current_page_option_ids = {int(opt.value) for opt in self.paginator.pages[self.paginator.current_page]}
    ...
    async def on_back(self, interaction: Interaction):
        ...
        await self.parent_view_message.edit(content="在此管理服务器的聊天设置：", view=main_view, embed=None)
        # 停止当前视图
        self.stop()
