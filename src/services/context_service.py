...
class ContextService:
    ...
    async def update_user_conversation_history(self, user_id: int, guild_id: int,
        ...
        await db_manager.update_ai_conversation_context(user_id, guild_id, current_history)
    
    async def get_channel_conversation_history(self, channel_id: int, limit: int = 30) -> List[Dict]:
        """
    ...
    async def get_formatted_channel_history(self, channel_id: int, limit: int = 20, exclude_message_id: Optional[int] = None) -> List[Dict[str, any]]:
        ...
        try:
            ...
            anchor_message_id = await db_manager.get_channel_memory_anchor(guild_id, channel_id)
