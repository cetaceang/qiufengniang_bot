...
class GhostCardService:
    ...
    def player_draw_card(self, game_id: str, card_index: int) -> Tuple[bool, str, Optional[str], Optional[str]]:
        ...
        # 抽牌后的反应（保留欺骗逻辑）
        deception_failed = False
        last_deception = game.get("last_deception_type")
        if (last_deception == 'deceive_ghost' and is_ghost) or \
           (last_deception == 'deceive_safe' and not is_ghost):
            deception_failed = True
        
        # 检查AI手上是否有鬼牌（在抽牌后，但此时AI手牌已经更新）
        ai_has_ghost = "🃏" in game["ai_hand"]
        
        reaction_text, reaction_image_url, _ = self._get_bot_reaction(
            "drawn", is_ghost, game["ai_strategy"], ai_has_ghost, deception_failed=deception_failed
        )
