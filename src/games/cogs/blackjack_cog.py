...
from src.games.services.blackjack_service import BlackjackService
from src.games.ui.blackjack_ui import BlackjackView
from src.games.ui.bet_modal import BetModal
from src.odysseia_coin.service.coin_service import CoinService
from src.games.config.text_config import TextConfig
...
class BlackjackCog(commands.Cog):
    ...
    async def blackjack(self, interaction: discord.Interaction):
        ...
        if not await self.coin_service.has_sufficient_coins(user_id, bet_amount):
            await interaction.followup.send(TextConfig.GHOST_CARD_NOT_ENOUGH_COINS.format(bet_amount=bet_amount), ephemeral=True)
            return

        await self.coin_service.deduct_coins(user_id, bet_amount)
    ...
    async def end_game(self, interaction: discord.Interaction, game: BlackjackService, deferred: bool = False):
        ...
        if result in ["dealer_bust", "player_win"]:
            await self.coin_service.add_coins(user_id, game.bet_amount * 2)
        elif result == "push":
            await self.coin_service.add_coins(user_id, game.bet_amount)
