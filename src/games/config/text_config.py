...
class EmotionImageUrls:
    ...
    HAPPY = "https://cdn.discordapp.com/attachments/1403347767912562728/1404427399453741126/ComfyUI_temp_rppad_00478_.png?ex=68b238b1&is=68b0e-731&hm=f17cdc9169bcb89d14254ed4290c9aae1ec50f06c41f9ee3de0e6e7a2edf97c0&"
...
class ReactionPool:
    ...
    DRAWN_GHOST_HAPPY = Reaction(
        texts=["太好了！你终于把它抽走了！", "嘿嘿，这张烫手的山芋现在是你的了！祝你好运哦～", "**计划通！**"],
        image_url=EmotionImageUrls.HAPPY
    )
    ...
    AI_DRAWN_GHOST_SAD = Reaction(
        texts=["怎么会这样？！我算错了吗？", "不——！我的完美计划！", "呜...我居然抽到了这张牌..."],
        image_url=EmotionImageUrls.SAD
    )
    AI_DRAWN_SAFE_HAPPY = Reaction(
        texts=["嘿嘿，安全上垒！", "哦耶!抽到安全牌啦!", "一切都在计划之中!"],
        image_url=EmotionImageUrls.HAPPY
    )

    # 取消选择后的反应
    CANCELLED_GHOST_DISAPPOINTED = Reaction(
        texts=["哎，怎么不抽了？就差一点了...", "真可惜...", "改变主意了吗？再考虑一下嘛。"],
        image_url=EmotionImageUrls.SAD
    )
    CANCELLED_SAFE_RELIEVED = Reaction(
        texts=["改变主意了？太好了。", "嗯，谨慎一点总是好的。", "好的，你决定不抽这张了。"],
    ...
    CANCELLED_SAFE_FAKE_DISAPPOINTED = Reaction(
        texts=["什么嘛，好可惜，差点就把那张牌送出去了。", "啊啊啊啊可恶，就差一点点！", "真可惜啊..."],
        image_url=EmotionImageUrls.SAD
    )
    ...
    DECEPTION_FAILED = Reaction(
        texts=[
            "什...什么？！你居然看穿了我的计谋！", "不可能！我的演技应该天衣无缝才对！",
            "呃...好吧，算你厉害...", "呜...被你看穿了...", "失败了...", "哼，别得意！"
        ],
        image_url=EmotionImageUrls.SAD
    )
    PLAYER_LOST_WIN = Reaction(
        texts=["嘿嘿，一切都在我的计划之中！", "杂鱼!是我赢了哦!", "**胜利的方程式，完成了!**"],
        image_url=EmotionImageUrls.SUPER_WIN
    )
    PLAYER_LOST_CHEATING = Reaction(
        texts=["我...我才没输呢！这局不算！", "哼，刚刚是你看错了，重来！", "不算不算，这局不算！是你作弊！"],
        image_url=EmotionImageUrls.SAD
    )
...
class TextConfig:
    ...
    BLACKJACK_PLAYER_BUST: str = "你的点数超过21点，爆牌了！你输掉了 {bet_amount} 奥德赛币。"
    BLACKJACK_DEALER_BUST: str = "庄家爆牌了！你赢得了 {bet_amount} 奥德赛币！"
    BLACKJACK_PLAYER_WIN: str = "恭喜！你的点数更高，你赢得了 {bet_amount} 奥德赛币！"
    BLACKJACK_DEALER_WIN: str = "很遗憾，庄家的点数更高。你输掉了 {bet_amount} 奥德赛币。"
    ...
    class ConfirmModal:
        """确认抽牌弹窗的文本"""
        title: str = "确认抽牌"
        special_card_warning: str = "**这张牌感觉有点不一样...** 说不定是关键牌哦，确定要抽吗？"
    ...
    class AIReactions:
        ...
        reactions_map: Dict[str, Reaction] = field(default_factory=lambda: {
            ...
            "drawn_safe_deception_failed": ReactionPool.DECEPTION_FAILED,
            # 情况5: 游戏结束
